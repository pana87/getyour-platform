...
module.exports = class AuthServer {
  ...
  // static setSessionTimeout(id, minutes) {
  //   const interval = setTimeout( async () => {
  //     const {user} = await User.find(it => it.email.value === id)
  //     console.log(user);
  //     // remove session digest from user
  //     // get user
  //     // update digest to undefined
  //   }, 1000)
  ...
  static async verifyAssertion(options) {
    ...
    if (cData.origin !== clientLocation.origin) {
      Notification.error(SECURITY_WARNING)
      return {
        status: 500,
    ...
    const expectedRPIDHash = this.hash("sha256", clientLocation.hostname)
  ...
  static async verifyAttestation(options) {
    ...
    if (decodedClientDataJSON.origin !== clientLocation.origin) {
