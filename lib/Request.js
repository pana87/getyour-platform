                      ...
                      if (user.id === req.jwt.id) {
                        if (user[req.location.platform] !== undefined) {
                          if (user[req.location.platform].cart !== undefined) {
                            ...
                            for (let i = 0; i < user[req.location.platform].cart.length; i++) {
                              const cart = user[req.location.platform].cart[i]

                              // item.tag = pv-module
                              // item.id = location-list-id
                              console.log(cart);
                              for (let i = 0; i < doc.users.length; i++) {
                                const user = doc.users[i]

                                if (user[req.location.platform] !== undefined) {
                                  if (user[req.location.platform][cart.tag] !== undefined) {
                                    for (let i = 0; i < user[req.location.platform][cart.tag].length; i++) {
                                      const locationListItem = user[req.location.platform][cart.tag][i]
                                      ...
                                      if (locationListItem.created === cart.id) {
                                      }
