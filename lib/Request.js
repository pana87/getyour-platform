...
module.exports.Request = class {
    ...
    try {
      ...
      if (req.params.method === "get") {
        ...
        if (req.params.type === "user") {
          ...
          if (req.params.event === "children-self") {

            const array = await Helper.get("keys/children", req)
            if (array.length > 0) return res.send(array)
            // deprecated when Helper is working
            // if (req.jwt !== undefined) {
            //   const array = []
            //   const doc = await nano.db.use("getyour").get("users")
            //   for (let i = 0; i < doc.users.length; i++) {
          ...
          if (req.params.event === "closed") {
            ...
            if (req.jwt !== undefined) {
              ...
              for (let i = 0; i < doc.users.length; i++) {
                const user = doc.users[i]
                if (user.id === req.jwt.id) {
                  if (user.scripts === undefined) user.scripts = []
                  const script = {}
                  script.created = Date.now()
                  script.name = req.body.name
                  script.html = req.body.html
                  user.scripts.unshift(script)
                  await nano.db.use("getyour").insert({ _id: doc._id, _rev: doc._rev, users: doc.users })
                  return res.sendStatus(200)
                }
              }
            ...
            if (req.jwt !== undefined) {
              if (req.location !== undefined) {
                ...
                for (let i = 0; i < doc.users.length; i++) {
                  const user = doc.users[i]
                  if (user.id === req.jwt.id) {
                    if (await Helper.verifyIs("user/location-writable", {user, req})) {
                      return res.sendStatus(200)
                    }
                  }
                }
  ...
  static async verifyJwtToken(req, res, next) {
    try {
      ...
      // function verifyIsJwtTokenEmpty(req, res) {
      //   console.log("verifyIsJwtTokenEmpty");
      ...
      if (req.method === "GET") {
        console.log("verifyJwtToken in get");
