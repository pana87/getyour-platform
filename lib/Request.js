            ...
            if (req.location !== undefined) {
              if (req.jwt !== undefined) {
                ...
                if (req.body.type !== undefined) {
                  ...
                  if (req.body.type === "mp3-binaries/cids/self") {

                    console.log(req.body);
                    console.log(req.files);
                    if (Helper.stringIsEmpty(req.body.mp3)) throw new Error("req.body.mp3 is empty")

                    // if binariess are ok
                    // Helper.convert("binary/cid", for each binary in binaries)
                  ...
                  if (req.body.type === "video-url/audio-cid/self") {

                    if (Helper.stringIsEmpty(req.body.videoUrl)) throw new Error("req.body.videoUrl is empty")
                    ...
                    for (let i = 0; i < doc.users.length; i++) {
                      ...
                      if (user.id === req.jwt.id) {
                        ...
                        const cid = await Helper.convert("video-url/audio-cid", req.body.videoUrl)

                        if (user["getyour"] === undefined) user["getyour"] = {}
                        if (user["getyour"].soundbox === undefined) user["getyour"].soundbox = []
                        user["getyour"].soundbox.unshift({ cid })
                      ...
                      if (user.id === req.jwt.id) {
                        ...
                        if (user["soundbox"] !== undefined) {
                          for (let i = 0; i < user["soundbox"].length; i++) {
                            ...
                            map.cid = sound.cid
                            map.author = sound.author
                            map.title = sound.title
