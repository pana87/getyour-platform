    ...
    try {
      ...
      if (req.params.method === "get") {
        ...
        if (req.params.type === "platform") {
          ...
          if (req.params.event === "url-id") {
            ...
            if (req.location !== undefined) {
              ...
              const doc = await nano.db.use("getyour").get("users")
              const urlUser = doc.users.find(it => it.id === req.body.urlId)
              if (urlUser && urlUser[req.location.platform]) {
                return res.send(urlUser[req.location.platform])
              }
            }
          }
        ...
        if (req.params.type === "users") {
          ...
          if (req.params.event === "tree") {

            if (Helper.verifyIs("text/empty", req.body.tree)) throw new Error("req.body.tree is empty")
            if (Helper.verifyIs("text/empty", req.body.algo)) throw new Error("req.body.algo is empty")
            const array = []
            const tree = req.body.tree
            const key = Helper.convert("tree/key", req.body.tree)
            const doc = await nano.db.use("getyour").get("users")
            for (let i = 0; i < doc.users.length; i++) {
              const user = doc.users[i]
              const map = {}
            ...
            Helper.sort(req.body.algo, array)
            if (array.length > 0) return res.send(array)
          }
    ...
    try {
      ...
      if (req.params.method === "register") {
        ...
        if (req.params.type === "contacts") {
          ...
          if (req.params.event === "notes") {

            if (req.jwt !== undefined) {
              if (Helper.verifyIs("number/empty", req.body.created)) throw new Error("req.body.created is empty")
              if (!Helper.isLengthValid(req.body.notes)) throw new Error("req.body.notes too long")
              const doc = await nano.db.use("getyour").get("users")
              for (let i = 0; i < doc.users.length; i++) {
                const user = doc.users[i]
                if (user.id === req.jwt.id) {
                  if (user.contacts !== undefined) {
                    for (let i = 0; i < user.contacts.length; i++) {
                      const contact = user.contacts[i]
                      if (contact.created === req.body.created) {
        ...
        if (!Helper.verifyIs("text/empty", req.params.type)) {
          ...
          if (!Helper.verifyIs("text/empty", req.params.event)) {
            if (req.jwt !== undefined) {
              if (!Helper.isReserved(req.params.type)) throw new Error(`${req.params.type} is not reserved`)
              const doc = await nano.db.use("getyour").get("users")
              if (Helper.verifyIs("number/empty", req.body.created)) {
                if (Helper.verifyIs("object/empty", req.body[req.params.event])) throw new Error(`req.body.${req.params.event} is empty`)
                for (let i = 0; i < doc.users.length; i++) {
                  const user = doc.users[i]
                  if (user.id === req.jwt.id) {
                    if (user[req.params.type] === undefined) user[req.params.type] = []
                    const map = {}
                    map[req.params.event] = req.body[req.params.event]
                    map.created = Date.now()
                    map.visibility = "closed"
                    const sorted = Helper.sort("keys/abc", map)
                    user[req.params.type].unshift(sorted)
                    await nano.db.use("getyour").insert({ _id: doc._id, _rev: doc._rev, users: doc.users })
                    return res.sendStatus(200)
                  }
                }
