                ...
                if (req.body.type !== undefined) {
                  ...
                  if (req.body.type === "id-self") {
                    ...
                    for (let i = 0; i < doc.users.length; i++) {
                      ...
                      if (user.id === req.jwt.id) {
                        if (user["getyour"] !== undefined) {
                          if (user["getyour"].templates !== undefined) {
                            for (let i = 0; i < user["getyour"].templates.length; i++) {
                              const template = user["getyour"].templates[i]
                              if (template.created === req.body.id) {
                                user["getyour"].templates.splice(i, 1)
                                await nano.db.use("getyour").insert({ _id: doc._id, _rev: doc._rev, users: doc.users })
                                return res.sendStatus(200)
                              }
                            }
                      ...
                      if (user.id === req.jwt.id) {
                        if (user.contacts !== undefined) {
                          for (let i = 0; i < req.body.contacts.length; i++) {
                            const newContact = req.body.contacts[i]
                            const alreadyExists = user.contacts.some(contact => {
                              return contact.created === newContact.created || contact.email === newContact.email
                            })
                            if (!alreadyExists) {
                              const map = {
                      ...
                      if (jwtUser.id === req.jwt.id) {
                        ...
                        for (let i = 0; i < doc.users.length; i++) {
                          ...
                          if (user.email === req.body.email) {
                            ...
                            user[location][req.body.id].push(obj)
                        ...
                        for (const key in req.body.map) {
                          if (req.body.map.hasOwnProperty(key)) {
                            if (user[location] === undefined) user[location] = {}
                            if (user[location][req.body.id] === undefined) user[location][req.body.id] = []
                            user[location][req.body.id].push(req.body.map[key])
                          }
                        }
                  ...
                  if (req.body.type === "html-self") {
                    ...
                    for (let i = 0; i < doc.users.length; i++) {
                      ...
                      if (user.id === req.jwt.id) {
                        ...
                        if (user["getyour"].templates === undefined) user["getyour"].templates = []

                        const template = {}
                        template.created = Date.now()
                        template.html = req.body.html
                        user["getyour"].templates.unshift(template)
                          ...
                          if (user.email === req.body.email) {
                            ...
                            for (const key in req.body.map) {
                              if (req.body.map.hasOwnProperty(key)) {
                                if (user[location] === undefined) user[location] = {}
                                if (user[location][req.body.id] === undefined) user[location][req.body.id] = {}
                                user[location][req.body.id][key] = req.body.map[key]
                              }
                            }
                  ...
                  if (req.body.type === "self") {
                    for (let i = 0; i < doc.users.length; i++) {
                      ...
                      if (user.id === req.jwt.id) {
                        if (user["getyour"] !== undefined) {
                          if (user["getyour"].templates !== undefined) {
                            return res.send(user["getyour"].templates)
                          }
                        }
                      }
                    }
