...
module.exports.Request = class {
  ...
  static async verifyLocation(req, res, next) {
    try {
      ...
      throw new Error("verify location failed")
    } catch (error) {
                      ...
                      if (user["getyour"].expert !== undefined) {
                        if (user["getyour"].expert.templates !== undefined) {
                          for (let i = 0; i < user["getyour"].expert.templates.length; i++) {
                            ...
                            if (template.id === req.body.id) {
                              template.script = req.body.scripts
              ...
              if (user.id === req.jwt.id) {
                ...
                if (user["getyour"] !== undefined) {
                  ...
                  if (user["getyour"].expert.name === url.expert) {
                    ...
                    //   if (Helper.stringIsEmpty(req.body.loadmode)) throw new Error("req.body.loadmode is empty")
                    //   template.loadmode = req.body.loadmode
                    // }
                    template.script = req.body.script
                    // template.owner = user.id

                    // do this on save feedback function
                    // template.feedback = []
                    // template.history = []
          ...
          if (req.params.event === "closed") {
            ...
            if (req.body.type !== undefined) {
              ...
              if (req.body.type === "html") {
                ...
                //   if (url.platform !== "getyour") throw new Error("platform mismatch")
                //   if (url.expert !== "pana") throw new Error("expert mismatch")
                //   if (url.path !== "toolbox-alpha") throw new Error("path mismatch")
                //   for (let i = 0; i < doc.users.length; i++) {
                //     const user = doc.users[i]
                //     if (user.id === jwt.id) {
                //       if (user["getyour"] !== undefined) {
                //         if (user["getyour"].expert.name === "pana") {
                //           for (let i = 0; i < user["getyour"].expert.platforms.length; i++) {
                //             const platform = user["getyour"].expert.platforms[i]
                //             if (platform.name === "getyour") {

                //               if (platform["toolbox-alpha"] === undefined) platform["toolbox-alpha"] = []
                //               const map = {}
                //               map.created = Date.now()
                //               map.licence = "open"
                //               map.type = "text/html"
                //               map.html = req.body.html
                //               platform["toolbox-alpha"].unshift(map)
                //               await nano.db.use("getyour").insert({ _id: doc._id, _rev: doc._rev, users: doc.users })
                //               return res.sendStatus(200)
                      ...
                      if (user["getyour"].expert.name === url.expert) {
                        for (let i = 0; i < user["getyour"].expert.platforms.length; i++) {
                          ...
                          if (platform.name === url.platform) {
                            for (let i = 0; i < platform.values.length; i++) {
                              const value = platform.values[i]
                              if (value.path === `/${url.platform}/${url.expert}/${url.path}/`) {
                          ...
                          if (platform.name === req.body.platform) {
                            for (let i = 0; i < platform.values.length; i++) {
                              ...
                              if (value.path === req.body.oldPath) {
                                value.path = `/${req.body.platform}/${user["getyour"].expert.name}/${req.body.newPath}/`
                                await nano.db.use("getyour").insert({ _id: doc._id, _rev: doc._rev, users: doc.users })
                                return res.sendStatus(200)
                              }
                            }
          ...
          if (req.params.event === "closed") {
            ...
            if (req.body.type !== undefined) {
              ...
              if (req.body.type === "new") {
                ...
                // const {location} = req.body
                // if (Helper.stringIsEmpty(req.body.location)) throw new Error("req.body.location is empty")
                // const loc = new URL(req.body.location)
                // if (Helper.objectIsEmpty(loc)) throw new Error("location is not valid")
                // ver
                // verify location in verifyEvent or one before verifyLocation()
                const url = {}
                url.expert = new URL(req.body.location).pathname.split("/")[1]
                if (Helper.stringIsEmpty(req.body.platform)) throw new Error("req.body.platform is empty")
                for (let i = 0; i < doc.users.length; i++) {
                    ...
                    if (user["getyour"].expert.name === url.expert) {
                      ...
                      for (let i = 0; i < user["getyour"].expert.platforms.length; i++) {
                        ...
                        if (platform.name === req.body.platform) {
                          ...
                          for (let i = 0; i < platform.values.length; i++) {
                            const value = platform.values[i]
                            if (value.path === `/${req.body.platform}/${url.expert}/${req.body.path}/`) {
                              return res.sendStatus(200)
                            }
                          }
                        }
  ...
  static async get(req, res, next) {
    try {
      ...
      if (req.params.method === "get") {
                    ...
                    if (user["getyour"].expert !== undefined) {
                      if (user["getyour"].expert.templates !== undefined) {
                        for (let i = 0; i < user["getyour"].expert.templates.length; i++) {
                          ...
                          // if (template.feedback === undefined) template.feedback = []
                          // map.feedback = template.feedback
        ...
        // first approach to get template script tags
        ...
        if (req.params.type === "platforms") {
          ...
          if (req.params.event === "closed") {
            ...
            if (Helper.stringIsEmpty(req.body.expert)) throw new Error("req.body.expert is emtpy")
            const array = []
            for (let i = 0; i < doc.users.length; i++) {
              const user = doc.users[i]
              if (user.id === jwt.id) {
