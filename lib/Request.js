            ...
            for (let i = 0; i < doc.users.length; i++) {
              ...
              if (user.id === jwt.id) {
                ...
                if (user.roles.length > 1) {
                  return res.send("/pana/getyour/rolle-wahlen/")
                }
                ...
                for (let i = 0; i < user.roles.length; i++) {
                  ...
                  if (role === UserRole.OPERATOR) {
                    ...
                    if (user[platform].funnel.hausBaujahr === undefined) return res.send("/felix/energie-plattform/abfrage-haus/1/")
                    return res.send("/felix/energie-plattform/anlagenbetreiber-ansicht/1/")
                  }
                }
              }
          ...
          if (req.params.event === "closed") {
            ...
            for (let i = 0; i < doc.users.length; i++) {
              ...
              if (user.id === jwt.id) {
                ...
                const {tag, title, description, message, notes, termsPdf, companyPdf, productPdf, discount, vat} = req.body
                ...
                const vatNumber = parseInt(vat)
                if (!Helper.numberIsEmpty(discountNumber)) user.offer.discount = discountNumber
                if (!Helper.numberIsEmpty(vatNumber)) user.offer.vat = vatNumber
      ...
      if (req.params.method === "get") {
              ...
              if (user.id === jwt.id) {
                for (let i = 0; i < user.roles.length; i++) {
                  ...
                  if (role === UserRole.OPERATOR) {
                    ...
                    map.path = `/felix/energie-plattform/anlagenbetreiber-ansicht/1/`
                  ...
                  if (role === UserRole.EXPERT) {
                    const map = {}
                    map.name = "Experten"
                    map.path = `/felix/energie-plattform/experten-ansicht/3/`
        ...
        if (req.params.type === "funnel") {
          if (req.params.event === "closed") {
            ...
            //   if (user.id === urlId) {
            //     if (user[tag] === undefined) throw new Error("funnel not found")
            //     return res.send(user[tag])
            //   }
