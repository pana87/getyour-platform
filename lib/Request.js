...
module.exports.Request = class {
          ...
          for (let i = 0; i < doc.users.length; i++) {
            ...
            if (user.id === jwt.id) {
              if (user["getyour"].expert.name === url.expert) {
                  if (user[url.platform].expert.platforms === undefined) throw new Error("platforms is undefined")
        ...
        if (req.params.type === "platform") {
          if (req.params.event === "closed") {
            ...
            for (let i = 0; i < reservedWords.length; i++) {
              ...
              if (req.body.platform === word) throw new Error("can not delete reserved word")
            }
            for (let i = 0; i < doc.users.length; i++) {
              const user = doc.users[i]
              if (user.id === jwt.id) {
              ...
              for (let i = 0; i < doc.users.length; i++) {
                ...
                if (user.id === jwt.id) {
                  ...
                  if (role === UserRole.EXPERT) {
                    if (Helper.stringIsEmpty(user[platform].expert.name)) throw new Error("expert.name is empty")
                    return res.send(`/getyour/${user[platform].expert.name}/`)
                  }
              ...
              await Helper.sendEmailFromDroid({
                ...
                html: /*html*/`
                  <div>
                    ...
                    Sie wurden von ${owner.firstname} ${owner.lastname} eingeladen, als Experte auf unserer Plattform teilzunehmen. <a href="https://get-your.de/pana/getyour/login/">Sie können sich jetzt ab sofort hier anmelden.</a><br/><br/>
          ...
          if (req.params.event === "closed") {
            ...
            if (req.body.type !== undefined) {
              ...
              if (req.body.type === "new") {
                if (Helper.stringIsEmpty(req.body.platform)) throw new Error("req.body.platform is empty")
                if (Helper.objectIsEmpty(req.body.logo)) throw new Error("req.body.logo is empty")
                if (Helper.stringIsEmpty(req.body.path)) throw new Error("req.body.path is empty")
                ...
                for (let i = 0; i < doc.users.length; i++) {
                  const user = doc.users[i]
                  if (user.id === jwt.id) {
                    if (user[url.platform].expert.name === url.expert) {
                      for (let i = 0; i < user[url.platform].expert.platforms.length; i++) {
                        const platform = user[url.platform].expert.platforms[i]
                        if (platform.name === req.body.platform) {

                          const value = {}
                          value.id = Helper.digest(`${Date.now()}`)
                          value.created = Date.now()
                          value.path = `/${req.body.platform}/${user[url.platform].expert.name}/${req.body.path}/`
                        ...
                        if (platform.name === req.body.platform) {
                          for (let i = 0; i < platform.values.length; i++) {
                            ...
                            if (value.path === req.body.oldPath) {
                              value.path = `/${req.body.platform}/${user[url.platform].expert.name}/${req.body.newPath}/`
                              await nano.db.use("getyour").insert({ _id: doc._id, _rev: doc._rev, users: doc.users })
                              return res.sendStatus(200)
                            }
                          }
                ...
                for (let i = 0; i < doc.users.length; i++) {
                  ...
                  if (user.id === jwt.id) {
                    if (user[url.platform].expert.name === url.expert) {
                      for (let i = 0; i < user[url.platform].expert.platforms.length; i++) {
                        const platform = user[url.platform].expert.platforms[i]
                        if (platform.name === req.body.name) {
                          for (let i = 0; i < platform.values.length; i++) {
                            const value = platform.values[i]
                            if (value.path === req.body.path) {
                              value.logo = req.body.logo
        ...
        if (req.params.type === "platform") {
          if (req.params.event === "closed") {
            ...
            if (type !== undefined) {
              ...
              if (type === "name") {
                if (Helper.stringIsEmpty(req.body.newName)) throw new Error("new name is empty")
                if (Helper.stringIsEmpty(req.body.oldName)) throw new Error("old name is empty")

                const reservedWords = Helper.getReservedWords()
                for (let i = 0; i < reservedWords.length; i++) {
                  const word = reservedWords[i]
                  if (req.body.newName === word) throw new Error("new name exist")
                }
                ...
                for (let i = 0; i < doc.users.length; i++) {
                  ...
                  if (user.id === jwt.id) {
                    if (user["getyour"].expert.name === url.expert) {
                      for (let i = 0; i < user["getyour"].expert.platforms.length; i++) {
                        const platform = user["getyour"].expert.platforms[i]
                        if (platform.name === req.body.oldName) {
                          for (let i = 0; i < platform.values.length; i++) {
                            const value = platform.values[i]
                            value.path = `/${req.body.newName}/${user["getyour"].expert.name}/${value.path.split("/")[3]}/`
                          }
                          platform.name = req.body.newName
                          await nano.db.use("getyour").insert({ _id: doc._id, _rev: doc._rev, users: doc.users })
                          return res.sendStatus(200)
                        }
                      }
              ...
              if (type === "logo") {
                if (Helper.objectIsEmpty(req.body.logo)) throw new Error("platform logo is empty")
                if (Helper.stringIsEmpty(req.body.name)) throw new Error("platform name is empty")
            ...
            for (let i = 0; i < doc.users.length; i++) {
              const user = doc.users[i]
              if (user[url.platform].expert.platforms !== undefined) {
                for (let i = 0; i < user[url.platform].expert.platforms.length; i++) {
                  if (user[url.platform].expert.platforms[i].name === name) throw new Error("name exist")
                }
              }
            }
            ...
            for (let i = 0; i < doc.users.length; i++) {
              ...
              if (user.id === jwt.id) {
                ...
                if (user[url.platform].expert.name === url.expert) {
                  ...
                  value.id = Helper.digest(`${Date.now()}`)
                  value.created = Date.now()
                  value.path = `/${name}/${user[url.platform].expert.name}/meine-erste-werteinheit/`
                  ...
                  user[url.platform].expert.platforms.push(map)
                  await nano.db.use("getyour").insert({ _id: doc._id, _rev: doc._rev, users: doc.users })
                  return res.sendStatus(200)
                }
      ...
      if (req.params.method === "get") {
        ...
        if (req.params.type === "platforms") {
          ...
          if (req.params.event === "closed") {
            ...
            const {type} = req.body
            if (type !== undefined) {
              if (Helper.stringIsEmpty(type)) throw new Error("type is emtpy")

              if (type === "values") {
                if (Helper.stringIsEmpty(req.body.name)) throw new Error("platform name is emtpy")
                for (let i = 0; i < doc.users.length; i++) {
                  const user = doc.users[i]
                  if (user.id === jwt.id) {
                    if (user[url.platform].expert.name === url.expert) {
              ...
              if (user.id === jwt.id) {
                if (user[url.platform].expert.name === url.expert) {
                  for (let i = 0; i < user[url.platform].expert.platforms.length; i++) {
                    const platform = user[url.platform].expert.platforms[i]
                    const map = {}
                    map.visibility = platform.visibility
                    map.logo = platform.logo.dataUrl
                    map.name = platform.name
                    // map.values = platform.values
                    array.push(map)
                  }
                }
              }
          ...
          if (req.params.event === "open") {
            ...
            if (Helper.stringIsEmpty(req.body.platform)) throw new Error("platform is empty")
            if (Helper.stringIsEmpty(req.body.expert)) throw new Error("expert is empty")


            const array = []
            for (let i = 0; i < doc.users.length; i++) {
              const user = doc.users[i]
              if (user[req.body.platform].expert.name !== undefined) {
                if (user[req.body.platform].expert.name === req.body.expert) {
                  for (let i = 0; i < user[req.body.platform].expert.platforms.length; i++) {
                    const platform = user[req.body.platform].expert.platforms[i]
  ...
  static async verifyEvent(req, res, next) {
    try {
      ...
      // allow only https://get-your.de or http://localhost:9999
    ...
    try {
      ...
      if (req.method === "GET") {
        ...
        if (Helper.stringIsEmpty(jwtToken)) {
          ...
          return res.redirect("/getyour/pana/login/")
        }
