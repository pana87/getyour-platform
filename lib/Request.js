...
module.exports.Request = class {
    ...
    try {
      ...
      if (type === "user") {
        if (method === "get") {
          ...
          // const html = Helper.jsonToHtml(doc.users)
          // console.log(html);
        ...
        if (method === "redirect") {
          ...
          for (let i = 0; i < user.roles.length; i++) {
            if (user.roles[i] === UserRole.OPERATOR) {
              if (user.checklists === undefined) return res.send("/felix/shs/qualifizierung/")
  ...
  static async verifyClosedPostRequest(req, res, next) {
    try {
      ...
      throw new Error("closed post request faile")
    } catch (error) {
      await Helper.logError(error, req)
    }
    return res.sendStatus(404)
  ...
  static async verifyProducerEmail(req, res, next) {
    try {
  ...
  // static async verifyOpenRegistration(req, res, next) {
  //   try {
