...
module.exports.Helper = class {
  ...
  // static jsonToHtml(json) {
  //   // Create an empty string to store our HTML
  //   let html = ""

  //   // Loop through the keys in the object
  //   for (const key in json) {
  //     if (json.hasOwnProperty(key)) {
  //       // Add the key and value to our HTML string
  //       html += `<div><strong>${key}:</strong> ${json[key]}</div>`;
  //     }
  //   }

  //   // Wrap our HTML in a container with some basic styling
  //   html = `<div style="font-family: sans-serif; font-size: 14px; line-height: 1.5em; color: #333; padding: 20px; border: 1px solid #ccc; border-radius: 5px;">${html}</div>`;
  ...
  static createSHSChecklist(options) {
    ...
    for (let i = 0; i < user.funnels.length; i++) {
      if (user.funnels[i].owner.id === jwt.id && user.funnels[i].selected === true) {
        checklist.owner.name = `${user.funnels[i].owner.firstname} ${user.funnels[i].owner.lastname}`
        found = true
        break
      }
    }
    if (found === false) throw new Error("funnel not found")
  ...
  static async logError(error, request) {
    try {
      ...
      if (request.method === "POST") {
        const {email, referer} = request.body
        log.email = email
        log.referer = referer
      }

      if (request.jwt !== undefined) {
        log.roles = request.jwt.roles
        log.jwtId = request.jwt.id
      }
  ...
  static async findUser(predicate) {
    return new Promise(async(resolve, reject) => {
      ...
      if (this.objectIsEmpty(doc)) return reject(new Error("doc is empty"))
      for (let i = 0; i < doc.users.length; i++) {
        if (this.booleanIsEmpty(predicate(doc.users[i]))) return reject(new Error("predicate is empty"))
        if (this.objectIsEmpty(doc.users[i])) return reject(new Error("user is empty"))
