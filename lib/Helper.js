...
module.exports.Helper = class {
  ...
  static createExpressServer() {
    ...
    return (route, port) => {
      ...
      let server
      function start() {
        server = app.listen(port, () => console.log(`[${route}] is running on port :${port}`))
      }
      function stop() {
        if (server) {
  ...
  static get(event, input) {
    ...
    if (event === "doc/db") {
      return new Promise(async(resolve, reject) => {
        try {
          const doc = await nano.db.use("getyour").get(input)
          if (this.objectIsEmpty(doc)) throw new Error("doc is empty")
          if (doc[input] === undefined) throw new Error(`${input} is undefined"`)
          resolve(doc)
        } catch (error) {
          reject(error)
        }
  ...
  static verifyIs(event, input) {
    ...
    if (event === "user/location-writable") {
      ...
      return new Promise(async(resolve, reject) => {
        try {
          const doc = await this.get("doc/db", "users")
