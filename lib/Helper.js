...
module.exports.Helper = class {
  ...
  static async findUser(predicate) {
    return new Promise(async(resolve, reject) => {
      ...
      if (this.objectIsEmpty(doc)) return reject("doc is empty")
      for (let i = 0; i < doc.users.length; i++) {
        if (this.booleanIsEmpty(predicate(doc.users[i]))) return reject("predicate is empty")
        if (this.objectIsEmpty(doc.users[i])) return reject("user is empty")
