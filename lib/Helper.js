...
module.exports.Helper = class {
  ...
  static verify(event, input, check) {
    ...
    if (event === "user/condition") {
      ...
      const value = check.left.split('.').reduce((acc, key) => acc[key], input)
  ...
  static convert(event, input) {
    ...
    if (event === "user-tree/value") {
      return input.tree.split('.').reduce((acc, key) => acc[key], input.user)
    }
    ...
    if (event === "mirror/users-map") {
      ...
      return new Promise(async(resolve, reject) => {
        ...
        // input = ["getyour.expert.name", "getyour.expert", "getyour"]
        // console.log(input);
        ...
        userloop: for (let i = 0; i < doc.users.length; i++) {
          ...
          for (let i = 0; i < input.length; i++) {
            ...
            // userObject.treeValues = []
            // if (this.verify("user/tree/exist", user, tree) !== true) continue
            ...
            if (this.verify("user/tree/exist", user, tree) === true) {
              ...
              const treeValuePair = {}
              // treeValuePair.reputation = user.reputation
              ...
              // userObject.treeValues.push(treeValuePair)

              // array.push(treeValuePair)
              // array.push(treeValuePair)

            }

            // array.push(clone[tree])
