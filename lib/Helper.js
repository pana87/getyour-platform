...
module.exports.Helper = class {
  ...
  static isReserved(string) {
    ...
    const reserved = ["id", "email", "verified", "reputation", "created", "getyour", "roles", "session", "owner", "expert", "admin"]
  ...
  static getReservedWords() {
    return ["id", "email", "verified", "reputation", "created", "getyour", "roles", "session", "owner", "expert", "admin"]
  }
    ...
    for (let i = 0; i < cookies.length; i++) {
      if (cookies[i] !== "jwtToken") {
        if (cookies[i] !== "sessionToken") {
          res.cookie(cookies[i], "", { expires: new Date.now() })
