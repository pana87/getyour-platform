...
module.exports.Helper = class {
  ...
  static checkExpiration() {
    ...
    // Calculate the future time in milliseconds (2 minutes from now
    const futureTimeMs = currentTimeMs + (2 * 60 * 1000)
    // Create a future date object using the future tim
    console.log("futureTimeMs", futureTimeMs);
    const futureDate = new Date(futureTimeMs)
    // Check if the future date has already passe
    console.log("futureDate", futureDate);
    const hasExpired = futureDate < new Date()
  ...
  static createSHSChecklist(options) {
    ...
    checklist.consumer = UserRole.OPERATOR
    checklist.name = "shs"
    ...
    checklist.path = {}
    checklist.path.home = `/felix/${checklist.name}/${checklist.type}/${jwt.id}/`
  ...
  static createBestprimeOffer(options) {
    ...
    offer.path = {}
    offer.path.home = "/felix/shs/match-maker/hersteller-vergleich/"
