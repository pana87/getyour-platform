...
module.exports.Helper = class {
  ...
  static verifyIs(event, input) {
    ...
    if (event === "email/super-admin") {
      ...
      if (input === "datenschutz@get-your.de") return true
      return false
    }
  ...
  static isReserved(string) {
    ...
    const reserved = ["id", "email", "verified", "reputation", "created", "getyour", "roles", "session", "owner", "expert", "admin", "type", "children", "parent"]
    ...
    for (let i = 0; i < cookies.length; i++) {
      if (cookies[i] !== "jwtToken") {
        if (cookies[i] !== "sessionToken") {
          ...
          // maybe break the app when other cookies spying
