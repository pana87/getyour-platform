...
// if i give the real database it should connect to real
...
module.exports.Helper = class {
  ...
  static convert(event, input) {
    ...
    if (event === "tree/value") {
      ...
      return getTreeValue(input.user, input.tree)
    }
  ...
  static api(event, input) {
    ...
    if (event === "gaming") {
      ...
      for (let i = 0; i < reserved.length; i++) {
        const word = reserved[i]
        if (input[word] === "created") continue
