...
module.exports.Helper = class {
  ...
  static convert(event, input) {
    ...
    // convert uploaded mp3 to cid
    if (event === "video-url/audio-cid") {
      return new Promise(async(resolve, reject) => {
        try {
          ...
          console.log(page)
          // Assuming your video is in a video element with the 'video' tag
          const videoElementHandle = await page.$('video')
          // console.log(videoElementHandle)

          // Get the source URL of the video
          const videoSource = await videoElementHandle.getAttribute('src')

          // console.log(videoSource)

          // Fetch the video data using the Fetch API
          ...
          if (res.status === 200) {
            ...
            // const videoMatch = res.data.match(/video/)
          ...
          return
          const videoDataBuffer = await response.buffer()

          // Create a Blob from the video data
          const videoBlob = new Blob([videoDataBuffer], { type: response.headers.get('content-type') })

          // Convert the video Blob to an audio Buffer using ffmpeg
          const audioBuffer = await new Promise((resolve, reject) => {
            ffmpeg()
              ...
              .pipe()
              .on('data', (chunk) => resolve(Buffer.from(chunk)))
              .on('end', () => resolve(Buffer.from([])))
          })

          // Now, audioBuffer contains the MP3 audio data
          console.log('MP3 audio buffer created:', audioBuffer)

          // Upload the audioBuffer to NFT.storage
          const nftStorageResponse = await fetch('https://api.nft.storage/upload', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer YOUR_NFT_STORAGE_API_KEY`, // Replace with your actual NFT.storage API key
            },
            body: JSON.stringify({
              content: audioBuffer.toString('base64'),
              name: 'audio.mp3', // Replace with the desired filename
            }),
          })

          const nftStorageResult = await nftStorageResponse.json()

          // Extract and log the CID
          const cid = nftStorageResult.cid
